...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_COMPOUND_EYES)
            calc = (calc * 4) / 3; // 1.3333 compound eyes boost
...
static void EndureDamage(bool8 checkFalseSwipe) {
    u8 holdEffect, param;
    bool8 falseSwipe, endured, focusBanded, sturdy;
    ...
    falseSwipe = checkFalseSwipe && gBattleMoves[gCurrentMove].effect == EFFECT_FALSE_SWIPE;
    endured = gProtectStructs[gBattlerTarget].endured;
    sturdy = gBattleMons[gBattlerTarget].ability == ABILITY_STURDY && gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP;
    ...
    if (!(gBattleMons[gBattlerTarget].status2 & STATUS2_SUBSTITUTE)
        ...
        && (falseSwipe || endured || sturdy || focusBanded)) {
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_WRAP:
                ...
                {
                    gBattleMons[gEffectBattler].status2 |= STATUS2_WRAPPED_TURN(5); // 4 turns
    ...
    if (gBattleOutcome == B_OUTCOME_WON)
    {
        ...
        else
        {
            ...
            case F_TRAINER_PARTY_DEFAULT:
                {
                    const struct TrainerMonDefault *party1 = gTrainers[gTrainerBattleOpponent_A].party.defaultMoves;
            ...
            case F_TRAINER_PARTY_CUSTOM_MOVES:
                {
                    const struct TrainerMonCustomMoves *party2 = gTrainers[gTrainerBattleOpponent_A].party.customMoves;
            ...
            case F_TRAINER_PARTY_CUSTOM_FULL:
                {
                    const struct TrainerMonCustomFull *party3 = gTrainers[gTrainerBattleOpponent_A].party.customFull;
